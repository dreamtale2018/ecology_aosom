...
import org.datacontract.schemas._2004._07.MH_ERPOAWcfService.UpdateOAApprovalStatusInfo;
import org.dom4j.Document;
import org.dom4j.DocumentException;
import org.dom4j.DocumentHelper;
import org.dom4j.Element;
import org.tempuri.erp.OAServiceContract;
import org.tempuri.erp.OAServiceContractProxy;
...
public class ERPManager {
	...
	private static final Log logger = LogFactory.getLog(ERPManager.class);
	 ...
	 * 
	 * @param type
	 * @param primaryValue单据号
	 * @param OAApprovalStatus状态
	 * @param OAApprovalUserName流程提交人
	 * @return 推送结果
	 * @author ycj@20190827
	 */
	public OracleResult<String, String> updateOAApprovalStatus(String type, String primaryValue, String OAApprovalStatus, String OAApprovalUserName, String task) {
		String code = "0";
		String message = "成功";
		String request = null;
		...
		try {
			...
			UpdateOAApprovalStatusInfo pinfo = new UpdateOAApprovalStatusInfo();
			pinfo.setType(type);
			pinfo.setPrimaryValue(primaryValue);
			pinfo.setOAApprovalStatus(OAApprovalStatus);
			pinfo.setOAApprovalUserName(OAApprovalUserName);
			pinfo.setName(username);
			pinfo.setPwd(password);
			response = proxy.updateOAApprovalStatus(pinfo);
	...
	private Map<String, Object> load() {
		...
		InputStream in = null;
		try {
			String path = Environment.getInstance().getAbsolutePathToProp("erp.properties");
//			String path = "E:\\Workspace\\ecology_aosen\\WebRoot\\ningb\\conf\\prop\\oracle.properties";
			...
			map.put("username", prop.get("erp.username"));
			map.put("password", prop.get("erp.password"));
	...
	private void loadCustom(Properties prop) {
		try {
			String queryName = Util.null2String(prop.get("erp.query.name"));
			...
			for (String query : queryArr) {
				...
				map.put("name", query);
				map.put("rminame", Util.null2String(prop.get("erp.query." + query + ".rminame")));
				map.put("primarykey", Util.null2String(prop.get("erp.query." + query + ".primarykey")));
				map.put("rmiargbean", Util.null2String(prop.get("erp.query." + query + ".rmiargbean")));
				map.put("rmiargname", Util.null2String(prop.get("erp.query." + query + ".rmiargname")));
				map.put("rmifiltername", Util.null2String(prop.get("erp.query." + query + ".rmifiltername")));
				map.put("rmifiltervalue", Util.null2String(prop.get("erp.query." + query + ".rmifiltervalue")));
				map.put("modeid", Util.null2String(prop.get("erp.query." + query + ".modeid")));
				map.put("modecreator", Util.null2String(prop.get("erp.query." + query + ".modecreator")));
