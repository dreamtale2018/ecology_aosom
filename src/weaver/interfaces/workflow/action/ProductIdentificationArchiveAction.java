...
/**
 * 认证需求表流程02节点提交后，禁出港口数据传入oracle<br>
 * 将数据推送给 Oracle
 * 
 * @author ycj
 *
 */
public class ProductIdentificationArchiveAction implements Action {
	
	private static final Log logger = LogFactory.getLog(ProductIdentificationArchiveAction.class);
				...
				for (int i = 0; i < detailAList.size(); i++) {
					...
					for(int j=0; j<gb.length; j++){
						...
						if(!"".equals(jcgkDetailA)){
							...
							if("0".equals(jjDetailA)){
								headContentMap.put("is_ban", "Y");
							}else{
								headContentMap.put("is_ban", "N");
							}
