 ...
 *
 */
public class ProductOrderJgtzArchiveAction implements Action {
	
	private static final Log logger = LogFactory.getLog(ProductOrderJgtzArchiveAction.class);
		...
		try {
			...
			if (detailAList != null && detailAList.size() > 0) {
				for (int i = 0; i < detailAList.size(); i++) {
					...
					String jqDetailA = Util.null2String(WorkflowUtils.getDetailFieldSelectName(workflowid, 1, "JQ", detailAMap.get("JQ")));		
																						// 交期
