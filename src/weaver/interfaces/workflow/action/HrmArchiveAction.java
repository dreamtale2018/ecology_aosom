 ...
 *
 */
public class HrmArchiveAction implements Action {

	private static final Log logger = LogFactory.getLog(HrmArchiveAction.class);
	...
	public String execute(RequestInfo request) {
		try {
			...
			String gx = mainTable.get("GX");			// 关系
			String jjlxrsjh = mainTable.get("JJLXRSJHM");
														// 紧急联系人手机号
			String bysj = mainTable.get("BYSJ");		// 毕业时间
			String ysxl = mainTable.get("YSXL");		// 原始学历
			String xx = mainTable.get("XX");			// 学校
			String zy = mainTable.get("ZY");			// 专业
			String sfzdz = mainTable.get("SFZDZ");		// 身份证地址
			String flzj = mainTable.get("FLZJ");		// 福利职级
			String gwzj = mainTable.get("GWZJ");		// 岗位职级
			String htqdcs = mainTable.get("HTQDCS");	// 合同签订次数
			String aqjb = mainTable.get("AQJB");		// 安全级别
			String yhkh = mainTable.get("YHKH");		// 银行卡号
			...
			customPersonMap.put("field18", flzj);
			customPersonMap.put("field19", gwzj);
			customPersonMap.put("field23", yhkh);
