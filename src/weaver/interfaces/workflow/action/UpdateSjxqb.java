...
/**
 * 设计需求表02节点从货号申请表带出图片<br>
 * 
 * @author ycj
 *
 */
public class UpdateSjxqb implements Action
{
  private Log logger = LogFactory.getLog(UpdateSjxqb.class);
		...
		if (detailAList != null && detailAList.size() > 0) {
			for (int i = 0; i < detailAList.size(); i++) {
				...
				if (rs.next()){
					...
					if(!"".equals(cptp)){
						String sqlTp = "update formtable_main_279_dt1 set tp = '" + cptp + "' where hh = '"+ hhDetailA +"'";
						rs.execute(sqlTp);
					}
				}
			}
