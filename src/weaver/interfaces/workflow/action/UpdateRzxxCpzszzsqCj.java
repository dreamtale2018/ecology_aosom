...
/**
 * 产品认证制作申请表创建节点新建认证信息台账<br>
 * 
 * @author ycj
 *
 */
public class UpdateRzxxCpzszzsqCj implements Action
{
  private Log logger = LogFactory.getLog(UpdateRzxxCpzszzsqCj.class);
    ...
    try
    {
		...
		if (detailAList != null && detailAList.size() > 0) {
			for (int i = 0; i < detailAList.size(); i++) {
				...
				String zslyDetailA = Util.null2String(detailAMap.get("FYGS"));			//证书来源
				if("0".equals(zslyDetailA)){
					zslyDetailA = "1";
				}else if("2".equals(zslyDetailA)){
					zslyDetailA = "0";
				}else{
					zslyDetailA = "";
				}
