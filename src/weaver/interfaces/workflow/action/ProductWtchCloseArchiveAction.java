...
/**
 * 问题存货/在制处理单流程供应链助理节点提交后，明细数据传入oracle关闭合同行<br>
 * 归档, 将数据推送给 Oracle
 * 
 * @author ycj
 *
 */
public class ProductWtchCloseArchiveAction implements Action {
	
	private static final Log logger = LogFactory.getLog(ProductWtchCloseArchiveAction.class);
		...
		try {
			...
			if (detailAList != null && detailAList.size() > 0) {
				for (int i = 0; i < detailAList.size(); i++) {
					...
					String sfgbDetailA = Util.null2String(detailAMap.get("HTHSFXGB"));							// 合同行是否需关闭
					if("0".equals(hztDetailA) && "0".equals(sfgbDetailA)){
						...
						String wtlxDetailA = Util.null2String(WorkflowUtils.getDetailFieldSelectName(workflowid, 1, "WTLX", detailAMap.get("WTLX")));								
																												// 问题类型
						...
						detailContentMap.put("remark", wtlxDetailA);
